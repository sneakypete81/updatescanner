...
describe('page_folder', function() {
  ...
  describe('loadChildren', function() {
    it('does nothing if the children list is undefined', function(done) {
      ...
      pageFolder.loadChildren().then((result) => {
    ...
    it('does nothing if there are no children', function(done) {
      const pageFolder = new PageFolder({children: []});
    ...
    it('loads child pages with no hierarchy', function(done) {
      const pageFolder = new PageFolder({id: 0, name: 'root',
                                         children: [1, 2]});
      ...
      pageFolder.loadChildren(loadPageSpy).then((result) => {
    ...
    it('loads child pages with subfolder hierarchy', function(done) {
      const pageFolder = new PageFolder({id: 0, name: 'root', children:
                                         [1, 2,
